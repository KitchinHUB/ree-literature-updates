...
in the Ames work Cu²⁺, with Fe³⁺ also demonstrated and found inferior
[@powell1956basic]. The reason copper works is worth stating carefully, because
it is often given backwards. Its EDTA complexes CuCh²⁻ and CuHCh⁻ have *very
large* formation constants, while the resin holds trivalent rare earths more
strongly than divalent copper; the two effects together give a large equilibrium
constant for the reaction in which chelated rare earths displace copper from the
resin. Eluent that runs ahead of the front is therefore intercepted by the
retaining ion and the rare earths redeposit within one or two theoretical
plates, so the front cannot smear: it stays a step. Powell and Spedding's own
general rule is that a retaining ion must either form a *more* stable soluble
EDTA complex than the rare earths do, or a slightly less stable one that the
resin holds less tightly — either way, it has to precede the rare earths down
the column. The copper bed has a limit: above eluant pH 8.4 the heaviest
lanthanides, Lu, Yb, Tm and Er, are not completely retained by it.
...
emerges. The one boundary figure a source verified for this chapter supplies is
Powell and Spedding's: the initial spread-out of the band, which is considerable
in citrate elutions, is under ten per cent of the original band length with EDTA
or HEDTA, and how far the band must then be pushed depends on the pair —
dysprosium and yttrium resolve in about two band lengths, yttrium and terbium
need nearly three [@powell1956basic]. No boundary width in millimetres or plates
is asserted. This is why the method reaches purities a cascade struggles with: the
...
| **Ion exchange (displacement)** — [](#displacement-chromatography) | Adjacent-pair exchange constants computed from EDTA stability constants run from 1.02 (Gd/Eu) to 4.8 (Tb/Gd), with **Nd/Pr = 2.0**; the HEDTA system gives 2.1 for Nd/Pr and 7.3 for Ce/La [@powell1956basic] | Real: mixed REE loaded on cation resin | Chelating eluent (EDTA/HEDTA/DTPA), ambient, Cu²⁺ retaining ion (Fe³⁺ also demonstrated and inferior; H⁺ suffices with citrate or HEDTA) | Commercial polishing step; pilot plants since 1947 [@spedding1947separation] | 9 [^trl-ix] |
