...
Generated by `python tools/verify_bib.py` on 2026-09-05. Re-run it whenever `references.bib` changes; the counts below are only as current as that run.

- Entries checked: **685**
- Verified: **685** (634 by resolving a CrossRef DOI, 2 by resolving a DataCite DOI, 46 by live URL, 3 by ISBN, 0 kept unchecked because nothing answered)
- Rejected: **0**
...
An earlier run rejected `iaea2011radiation` on the grounds that its URL
