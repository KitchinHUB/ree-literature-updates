...
- ~~No figures exist in any source document.~~ — resolved. Nine figures are
  drawn, each generated by a script in `tools/figures/` that writes an SVG to
  `figures/`, and each referenced by exactly one chapter: ch. 3 (log D vs pH,
  and the cascade), ch. 5 (flowsheet), ch. 8 (coacervate phase diagram), ch. 9
  (flow regimes, kinetics), ch. 10 (precipitation pH), ch. 11 (the lanmodulin
  EF-hand) and ch. 13 (the thermodynamic cycle). Every drawn number is sourced
  in the script's docstring; a figure asserts a number more strongly than prose
  does, so nothing is drawn that the bibliography cannot carry. Chapters 6, 7,
  12 and 15-20 still have none.
